...
#####################################################
# TELEMETRY TASKS
async def send_telemetry_from_temp_controller(client, telemetry_msg, component_name=None):
...
async def execute_command_listener(
    ...
    :param client: The device client
    ...
    while True:
        ...
        command_request = await client.receive_method_request(command_name)
        ...
        try:
            await client.send_method_response(command_response)
        except Exception:
...
#####################################################
# PROPERTY TASKS
def edge_twin_patch_handler(patch):
    print("the data in the desired properties patch was: {}".format(patch))
    properties_dict = pnp_helper.create_reported_properties_from_desired(patch)
    #await client.patch_twin_reported_properties(properties_dict)
...
async def main():
    ...
    property_updates = asyncio.gather(
        client.patch_twin_reported_properties(properties_root),
        client.patch_twin_reported_properties(properties_thermostat1),
        client.patch_twin_reported_properties(properties_thermostat2),
        client.patch_twin_reported_properties(properties_device_info),
    )
    ...
    listeners = asyncio.gather(
        execute_command_listener(
            client, method_name="reboot", user_command_handler=reboot_handler
        ),
        execute_command_listener(
            client,
    ...
    async def send_telemetry():
        ...
        while True:
            ...
            await send_telemetry_from_temp_controller(
                client, temperature_msg1, thermostat_1_component_name
            )
            ...
            await send_telemetry_from_temp_controller(
                client, temperature_msg2, thermostat_2_component_name
            )

            workingset_msg3 = {"workingSet": random.randrange(1, 100)}
            await send_telemetry_from_temp_controller(client, workingset_msg3)
